...
<body>
    ...
    if(count($_GET)==0){
    ?>
    <form method="get" action="zadanie-2.php">
    ...
    <?php
        ...
        if($_GET["haslo1_r"] === $_GET['haslo2_r']){
            ...
            header("Location: zadanie-2.php");
            exit;
        }
        }
            if(isset($_GET["login_l"]) && isset($_GET["haslo_l"])){
                if($_GET["login_l"] === $_COOKIE["LOGIN"] && $_GET["haslo_l"] === $_COOKIE["HASLO"]){
                    echo "POPRAWNE<br>" . "Login: " . $_COOKIE["LOGIN"]. "<br>Hasło: " . $_COOKIE["HASLO"] . "<br><a href='zadanie-2.php'>Wyloguj</a>";
                }
                else{
